...
""" This is a copy version of DQN_CartPole_Env.py, 
which is used for ablation study.
this code needs to be updated with hyperparameters, 
which you want to study. Edit the values in CONFIG and
 implementation()"""
...
# Training function
def CartPoleAgent(env, use_target_qnetwork=False, use_experience_replay=False, utd = None):
...
def implementation():
    ...
    for lr in study:
        print(f"############### Running for UTD: {lr} ###############")
        for r in range(reps):
            print(f"\nStarting Run {r + 1}/{reps}\n")

            print("############### Training Only Naive ##############")
            variants[lr]["Naive"].append(CartPoleAgent(env, use_target_qnetwork= False, use_experience_replay= False, utd = lr))

    mean = {
        lr: {key: np.mean([r for r in runs if r], axis=0) for key, runs in variants[lr].items()}
        for lr in study
    }
    std_deviations = {
        lr: {key: np.std([r for r in runs if r], axis=0) for key, runs in variants[lr].items()}
        for lr in study
    }
    steps = np.arange(0, len(mean[study[0]]["Naive"])) * (CONFIG["INTERVAL"] / 1000)

    # Plotting the graph
    plt.figure(figsize=(12, 8))
    window_size = 10
    for lr in study:
        for i in variants[lr].keys():
            if mean[lr][i].size > 0:
                if len(mean[lr][i]) >= window_size:
                    smoothed_rewards = np.convolve(mean[lr][i], np.ones(window_size) / window_size, mode='valid')
                    ...
                    plt.plot(smoothed_steps, smoothed_rewards, label=f"{i} (epsilon_decay={lr})", marker='o', linestyle='-', linewidth=2)
                    plt.fill_between(smoothed_steps, smoothed_rewards - std_deviations[lr][i],
                                     smoothed_rewards + std_deviations[lr][i], alpha=0.2)

    plt.xlabel('Steps (x1000)')
    plt.ylabel('Mean-Reward')
    plt.title('Naive Q-Learning')

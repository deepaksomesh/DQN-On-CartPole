...
""" This file is a copy of DQN_CartPole_Env.py, 
This code contains the logic of Double DQN at line 119."""
    ...
    for steps in range(CONFIG["MAX_STEPS"]):
        ...
        if steps % 10 == 0:
            ...
            with torch.no_grad():
                ...
                next_q_values = local_qnetwork(next_states_exp).argmax(dim=1)
                if use_experience_replay or (use_target_qnetwork and use_experience_replay):
                    next_qMax = target_qnetwork(next_states_exp).gather(1, next_q_values.unsqueeze(1)).squeeze(1)
...
def implementation():
    ...
    steps = np.arange(0, len(mean["tn_n_er"])) * (CONFIG["INTERVAL"] / 1000)
